...
4. **用量显示**：用量信息会在以下位置显示（按可用性自动选择）：
   - **计划与用量**设置页顶部：详细卡片（进度条、重置日期、倒计时）
   - **窗口右下角浮动条**：当状态栏隐藏时（Cursor Glass 布局默认隐藏状态栏），在设置齿轮图标左侧显示紧凑用量
   - **状态栏右侧**：若已启用状态栏（设置 → 外观 → 显示状态栏），则同时显示在状态栏
   - 悬停查看详情，点击刷新，每 60 秒自动刷新
